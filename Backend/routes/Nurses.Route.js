...
router.delete("/all", async (req, res) => {
  try {
    const result = await NurseModel.deleteMany({});
    res.status(200).send({ message: "All nurse data deleted", deletedCount: result.deletedCount });
  } catch (error) {
    console.error("Error deleting all nurse data:", error);
    res.status(500).send({ error: "Failed to delete all nurse data" });
  }
});
